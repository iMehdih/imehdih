...
export default function AdminWalletPage() {
  ...
  if (loading) return <div className="admin-page" style={{ textAlign: 'center', padding: '60px', color: '#505062' }}>در حال بارگذاری...</div>
  if (!data) return null

  return (
    <div className="admin-page">
      {msg && (
        <div className="admin-alert" style={{ marginBottom: 16, padding: '12px 16px', borderRadius: 10, fontSize: 13, background: msg.includes('خطا') ? 'rgba(239,68,68,0.08)' : 'rgba(34,197,94,0.08)', border: `1px solid ${msg.includes('خطا') ? 'rgba(239,68,68,0.2)' : 'rgba(34,197,94,0.2)'}`, color: msg.includes('خطا') ? '#EF4444' : '#22C55E' }}>
          {msg}
        </div>
      )}
        ...
        <div className="admin-card">
          ...
          <div style={{ padding: '12px 16px', display: 'flex', flexDirection: 'column', gap: 12 }}>
            {(data.staff || []).map((s: any) => (
              <div key={s._id} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '12px 14px', background: '#141420', borderRadius: 10 }}>
                ...
                <div style={{ flex: 1 }}>
                  ...
                  <div style={{ fontSize: 11, color: '#505062' }}>حقوق: {(s.salary / 1000000).toFixed(1)}م</div>
                </div>
                <div style={{ textAlign: 'left' }}>
                  <div style={{ fontSize: 16, fontWeight: 900, color: '#C8A96E' }}>{((s.walletBalance || 0) / 1000000).toFixed(1)}م</div>
                  <div style={{ fontSize: 10, color: '#505062' }}>موجودی</div>
                </div>
              </div>
            ))}
            {(data.staff || []).length === 0 && (
              <div style={{ textAlign: 'center', color: '#505062', fontSize: 12, padding: '20px 0' }}>کارمندی وجود ندارد</div>
            )}
          </div>
        </div>
              ...
              <div key={tx._id} style={{ padding: '14px 16px', borderBottom: '1px solid rgba(255,255,255,0.04)' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 6 }}>
                  <div>
                    ...
                    <div style={{ fontSize: 11, color: '#505062' }}>{tx.description}</div>
                  </div>
                  <div style={{ fontSize: 16, fontWeight: 900, color: '#C8A96E' }}>
                ...
                <div style={{ display: 'flex', gap: 8 }}>
                  <button
                    ...
                    style={{ flex: 1, padding: '7px', borderRadius: 8, border: 'none', background: '#22C55E', color: '#000', fontSize: 12, fontWeight: 800, cursor: 'pointer', fontFamily: 'inherit', opacity: actionLoading === tx._id ? 0.6 : 1 }}>
                    ✓ تأیید و پرداخت
                  </button>
                  <button
                    onClick={() => action({ action: 'reject_withdrawal', transactionId: tx._id }, tx._id + '_r')}
                    disabled={actionLoading === tx._id + '_r'}
                    style={{ padding: '7px 14px', borderRadius: 8, border: '1px solid rgba(239,68,68,0.3)', background: 'transparent', color: '#EF4444', fontSize: 12, fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit' }}>
                    رد
                  </button>
                </div>
              </div>
